...
# anthropometric variables, and significant metabolite features for the
# Akha, Lahu, CM and CR groups.
#
# The MFA integrates three groups of variables:
#   1. Metadata
#   2. Anthropometric variables
#   3. Significant metabolites
#
# Significant metabolites are selected based on adjusted p-values from
# the corresponding Wilcoxon rank-sum analyses.
...
# Set the working directory and load processed metabolomics datasets.
...
# ==========================================================
# 2. MFA – Akha Ethnic Group (Significant Metabolites)
# ==========================================================
# Load the Wilcoxon test results used to identify metabolites significantly
# associated with the Akha comparison.
wilcox_metabo_Akha <- read_delim("~/Documents/HillTribe_NGS/5.Metabolites/Metabo_Stat/wilcox_metabo_Akha.txt", 
                                 delim = "\t", escape_double = FALSE, 
                                 trim_ws = TRUE)


# Select significant metabolites ------------------------------------------------
# Retain metabolites with an adjusted p-value < 0.05 for inclusion in the MFA.
sig_Akha <- wilcox_metabo_Akha %>%
  filter(p_adj < 0.05) %>%
  pull(Metabolite)


# Prepare MFA dataset ----------------------------------------------------------
# Restrict the dataset to Akha participants and remove sample identifiers and
# variables that are not included in the MFA.
#
# Categorical variables are converted to factors. All metabolite variables
# are initially removed and then only the significant metabolites identified
# above are added back to the dataset.
Akha_MFA <- Met05_log2 %>%
  filter(Ethnicity == "Akha") %>%
  select(-Sample_ID, -Ethnicity, -BMl) %>%
  mutate(across(c(Gender, Group, Area, 
                  BMI_group, Hypertension), factor)) %>%
  select(-starts_with("Met"), all_of(sig_Akha)) # removes all metabolite columns, then  adds back only the significant metabolites


# Descriptive statistics -------------------------------------------------------
# Calculate the mean and standard deviation of numeric variables according to
# hypertension status.
Akha_MFA %>% 
  select(Group, Hypertension, Age:DBP) %>% 
...
# Run MFA ----------------------------------------------------------------------
# Integrate the three predefined groups of variables:
#
#   Group 1: Metadata
...
#
#   Group 3: Metabolite
#     245 quantitative metabolite variables
#
# The metadata group is defined as a supplementary group using
# num.group.sup = 1 and therefore does not contribute to the construction
...
# ==========================================================
# 2. MFA – Lahu Ethnic Group (Significant Metabolites)
# ==========================================================
# Perform MFA for the Lahu ethnic group using significant metabolites
# identified from the corresponding Wilcoxon analysis.
#
# The MFA integrates:
#   1. Metadata
#   2. Anthropometric variables
#   3. Significant metabolites
#
# The metadata group is treated as a supplementary group and therefore
# does not contribute to the construction of the principal dimensions.


# Load Wilcoxon test results ---------------------------------------------------
# Load the statistical results used to identify metabolites significantly
# associated with the Lahu comparison.
wilcox_metabo_Lahu <- read_delim("~/Documents/HillTribe_NGS/5.Metabolites/Metabo_Stat/wilcox_metabo_Lahu.txt", 
                                 delim = "\t", escape_double = FALSE, 
                                 trim_ws = TRUE)
...
# Select significant metabolites -----------------------------------------------
# Retain metabolites with an adjusted p-value < 0.05 for inclusion in the MFA.
sig_Lahu <- wilcox_metabo_Lahu %>%
  filter(p_adj < 0.05) %>%
  pull(Metabolite)


# Prepare MFA dataset ----------------------------------------------------------
# Restrict the dataset to Lahu participants and remove sample identifiers
# and variables that are not included in the MFA.
#
# Categorical metadata variables are converted to factors. All metabolite
# variables are initially removed, and only the significant metabolites
# identified above are added back to the dataset.
Lahu_MFA <- Met05_log2 %>%
  filter(Ethnicity == "Lahu") %>%
  select(-Sample_ID, -Ethnicity, -BMl) %>%
  mutate(across(c(Gender, Group, Area, 
                  BMI_group, Hypertension), factor)) %>%
  select(-starts_with("Met"), all_of(sig_Lahu)) # removes all metabolite columns, then  adds back only the significant metabolites


# Descriptive statistics -------------------------------------------------------
# Calculate the mean and standard deviation of numeric variables according to
# hypertension status as a descriptive check of the Lahu dataset.
Lahu_MFA %>% 
  select(Group, Hypertension, Age:DBP) %>% 
  group_by(Hypertension) %>% 
  summarise(
...
#
#   Group 3: Metabolite
#     47 quantitative metabolite variables
#
# The metadata group is specified as supplementary using num.group.sup = 1.
# Therefore, it is not used to construct the principal dimensions but can
# be used for interpretation of the resulting MFA structure.
...
# 3. MFA – CM Area
# ==========================================================
# Perform MFA for samples from Chiang Mai (CM) using metabolites identified
# as significantly different among the CM groups by the Kruskal-Wallis test.
#
# The MFA integrates:
#   1. Metadata
#   2. Anthropometric variables
#   3. Significant metabolites
#
# The metadata group is treated as a supplementary group and therefore does
# not contribute to the construction of the principal dimensions.


# Load Kruskal-Wallis results --------------------------------------------------
# Load the Kruskal-Wallis test results used to identify metabolites for
# inclusion in the CM MFA.
CM_kruskal_metabo <- read_delim("~/Documents/HillTribe_NGS/5.Metabolites/Metabo_Stat/CM_kruskal_metabo.txt", 
                                delim = "\t", escape_double = FALSE, 
                                trim_ws = TRUE)


# Select significant metabolites -----------------------------------------------
# Retain metabolites with a Kruskal-Wallis p-value < 0.05 for inclusion
# in the MFA.
sig_CM <- CM_kruskal_metabo %>%
  filter(kruskal_p < 0.05) %>%
  pull(Metabolite)


# Prepare MFA dataset ----------------------------------------------------------
# Restrict the dataset to CM samples and remove sample identifiers and
# variables that are not included in the MFA.
#
# Categorical variables are converted to factors. All metabolite variables
# are initially removed, and only the metabolites identified as significant
# by the Kruskal-Wallis analysis are added back.
CM_MFA <- Met05_log2 %>%
  filter(Area == "CM") %>%
  select(-Sample_ID, -Area, -BMl) %>%
  mutate(across(c(Gender, Group, Ethnicity, 
                  BMI_group, Hypertension), factor)) %>%
  select(-starts_with("Met"), all_of(sig_CM)) # removes all metabolite columns, then  adds back only the significant metabolites


# Descriptive statistics -------------------------------------------------------
# Calculate the mean and standard deviation of numeric variables according
# to study group as a descriptive check of the CM dataset.
CM_MFA %>% 
  select(Group, Hypertension, Age:DBP) %>% 
  group_by(Group) %>% 
  summarise(
...
# Calculate mean metabolite abundance by study group and gender for
# an additional descriptive check of Met52.
CM_MFA %>% 
  select(Group, Gender, Hypertension, Met52) %>%
...
#
#   Group 3: Metabolite
#     296 quantitative metabolite variables
#
# The metadata group is specified as supplementary using num.group.sup = 1.
# Therefore, metadata variables do not contribute to the construction of
# the principal dimensions but remain available for interpretation.
res.mfa <- MFA(CM_MFA,
...
# 4. MFA – CR Area
# ==========================================================
# Perform MFA for samples from Chiang Rai (CR) using metabolites identified
# as significantly different among the CR groups by the Kruskal-Wallis test.
#
# The same MFA structure used for the CM analysis is applied to the CR
# dataset, with the number of significant metabolites determined separately
# from the CR Kruskal-Wallis analysis.


# Load Kruskal-Wallis results --------------------------------------------------
# Load the Kruskal-Wallis test results used to identify metabolites for
# inclusion in the CR MFA.
CR_kruskal_metabo <- read_delim("~/Documents/HillTribe_NGS/5.Metabolites/Metabo_Stat/CR_kruskal_metabo.txt", 
                                delim = "\t", escape_double = FALSE, 
                                trim_ws = TRUE)
...
# Retain metabolites with a Kruskal-Wallis p-value < 0.05 for inclusion
# in the MFA.
sig_CR <- CR_kruskal_metabo %>%
  filter(kruskal_p < 0.05) %>%
  pull(Metabolite)


# Prepare MFA dataset ----------------------------------------------------------
# Restrict the dataset to CR samples and remove sample identifiers and
# variables that are not included in the MFA.
#
# Categorical variables are converted to factors. All metabolite variables
# are initially removed, and only the metabolites identified as significant
...
CR_MFA <- Met05_log2 %>%
  ...
  mutate(across(c(Gender, Group, Ethnicity, 
                  BMI_group, Hypertension), factor)) %>%
  select(-starts_with("Met"), all_of(sig_CR)) # removes all metabolite columns, then  adds back only the significant metabolites


# Descriptive statistics -------------------------------------------------------
# Calculate the mean and standard deviation of numeric variables according
# to study group as a descriptive check of the CR dataset.
CR_MFA %>% 
  select(Group, Gender, Hypertension, Age:DBP) %>% 
  group_by(Group) %>% 
  summarise(
    across(
      where(is.numeric),
...
# Calculate mean metabolite abundance by study group and hypertension status
# for an additional descriptive check of Met150.
CR_MFA %>% 
  select(Group, Hypertension, Met150) %>%
  group_by(Group, Hypertension) %>% 
  summarise(
    across(
      where(is.numeric),
      ~ mean(.x, na.rm = TRUE)
...
#
#   Group 3: Metabolite
#     122 quantitative metabolite variables
#
# The metadata group is specified as supplementary using num.group.sup = 1.
# Therefore, metadata variables do not contribute to the construction of
# the principal dimensions but remain available for interpretation.
...
fviz_mfa_ind(res.mfa, axes = c(1,2), 
             ...
             habillage = c("Group", "Hypertension"), # colored by groups
